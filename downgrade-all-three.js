...
// Known charger IPs from discovery
const KNOWN_CHARGER_IPS = [
    '10.10.10.176',
    '169.254.70.115',
    '10.110.73.155'
];
...
async function testDeviceConnection(ip) {
    return new Promise((resolve) => {
        const options = {
            ...
            method: 'GET',
            timeout: 5000  // Longer timeout for potentially different networks
        };
            ...
            res.on('end', () => {
                try {
                    ...
                    if (deviceName && deviceName.toLowerCase().includes('klvr')) {
                        ...
                        resolve({ ip, connected: false, error: 'Not a KLVR device' });
                    }
                } catch (error) {
                    console.log(`[DISCOVERY] Failed to parse response from ${ip}: ${error.message}`);
                    resolve({ ip, connected: false, error: 'Parse error' });
                }
            });
        });

        req.on('error', (error) => {
            console.log(`[DISCOVERY] Connection error for ${ip}: ${error.message}`);
            resolve({ ip, connected: false, error: error.message });
        });
        
        req.on('timeout', () => {
            req.destroy();
            console.log(`[DISCOVERY] Timeout connecting to ${ip}`);
            resolve({ ip, connected: false, error: 'Connection timeout' });
        });
...
async function downgradeAllThree() {
    const targetVersion = '1.8.3';

    try {
        console.log('🔍 Checking and Downgrading All Three Known Chargers');
        console.log('=' .repeat(60));
        console.log(`[FIRMWARE] Target version: ${targetVersion}`);
        console.log(`[FIRMWARE] Known charger IPs: ${KNOWN_CHARGER_IPS.join(', ')}`);
        
        // Step 1: Test connections to all devices
        console.log(`\n[DISCOVERY] Testing connections to all known chargers...`);
        const deviceInfos = [];
        
        for (const ip of KNOWN_CHARGER_IPS) {
            console.log(`[DISCOVERY] Checking ${ip}...`);
            const deviceInfo = await testDeviceConnection(ip);
            deviceInfos.push(deviceInfo);
            
            if (deviceInfo.connected) {
                console.log(`[DISCOVERY] ✅ Connected: ${deviceInfo.deviceName} at ${ip} (v${deviceInfo.firmwareVersion})`);
            } else {
                console.log(`[DISCOVERY] ❌ Failed: ${ip} - ${deviceInfo.error}`);
            }
        }
        
        // Step 2: Display found devices
        const connectedDevices = deviceInfos.filter(d => d.connected);
        const failedDevices = deviceInfos.filter(d => !d.connected);
        
        console.log(`\n📋 Connection Summary:`);
        console.log(`Connected: ${connectedDevices.length}/${KNOWN_CHARGER_IPS.length}`);
        console.log(`Failed: ${failedDevices.length}/${KNOWN_CHARGER_IPS.length}`);
        
        if (connectedDevices.length === 0) {
            console.log(`\n[ERROR] No devices could be connected. Please check network connectivity.`);
            return;
        }
        ...
        const devicesToDowngrade = connectedDevices.filter(device => 
            device.firmwareVersion !== targetVersion
        );
        
        const alreadyCorrectVersion = connectedDevices.filter(device => 
            device.firmwareVersion === targetVersion
        );
        ...
        if (devicesToDowngrade.length === 0) {
            console.log(`[FIRMWARE] All connected devices are already running version ${targetVersion}!`);
            return;
        }
        ...
        console.log(`Total known chargers: ${KNOWN_CHARGER_IPS.length}`);
        console.log(`Successfully connected: ${connectedDevices.length}`);
        ...
        if (failedDevices.length > 0) {
            console.log(`\n⚠️  Devices that could not be reached:`);
            failedDevices.forEach(device => {
                console.log(`  - ${device.ip}: ${device.error}`);
            });
        }
...
module.exports = { downgradeAllThree };
